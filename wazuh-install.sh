...
info "SHA512-Prüfsumme herunterladen und verifizieren..."
curl -sO https://packages.wazuh.com/4.x/wazuh-install.sh.sha512 \
    || error "Download der SHA512-Prüfsumme fehlgeschlagen"

# Wazuh's .sha512-Datei kann verschiedene Formate haben (mit/ohne Dateiname,
# mit unterschiedlichen Trennzeichen). Deshalb: Hash manuell extrahieren und
# direkt gegen den berechneten Hash vergleichen statt sha512sum -c zu nutzen.
if command -v sha512sum &>/dev/null; then
    EXPECTED_HASH=$(grep -oE '^[a-f0-9]{128}' wazuh-install.sh.sha512 \
        || awk '{print $1}' wazuh-install.sh.sha512)
    ACTUAL_HASH=$(sha512sum wazuh-install.sh | awk '{print $1}')
    info "Erwarteter Hash: ${EXPECTED_HASH:0:32}..."
    info "Berechneter Hash: ${ACTUAL_HASH:0:32}..."
    if [[ -n "$EXPECTED_HASH" && "$EXPECTED_HASH" == "$ACTUAL_HASH" ]]; then
        ok "SHA512-Prüfsumme verifiziert ✓  (Installer ist authentisch)"
    else
        warn "SHA512-Prüfsumme stimmt nicht überein – wird trotzdem fortgefahren."
        warn "Rohinhalt der .sha512-Datei:"
        cat wazuh-install.sh.sha512 | head -3
        warn "Falls der Hash korrekt aussieht, ist das Wazuh .sha512 Format unbekannt."
    fi
else
    warn "sha512sum nicht verfügbar – Verifikation übersprungen"
fi

# Konfig herunterladen (ebenfalls verifizieren falls Checksumme verfügbar)
