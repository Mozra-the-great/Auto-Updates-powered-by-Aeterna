...
if command -v sha512sum &>/dev/null; then
    EXPECTED_HASH=$(awk '{print $1}' wazuh-install.sh.sha512)
    ACTUAL_HASH=$(sha512sum wazuh-install.sh | awk '{print $1}')
    if [[ -n "$EXPECTED_HASH" && "$EXPECTED_HASH" == "$ACTUAL_HASH" ]]; then
        ok "SHA512-Prüfsumme verifiziert ✓  (Installer ist authentisch)"
    else
        echo ""
        error "SHA512-Prüfsumme STIMMT NICHT ÜBEREIN!
        Die heruntergeladene Datei könnte manipuliert worden sein.
        Installation wird abgebrochen. Bitte Verbindung und Quelle prüfen."
    fi
else
    warn "sha512sum nicht verfügbar – Verifikation übersprungen"
    warn "Prüfsumme manuell vergleichen: cat wazuh-install.sh.sha512"
fi
